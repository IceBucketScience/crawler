...
func handleIndexRequest(indexingJobCompletionQueue *msgQueue.DispatcherQueue) func(map[string]interface{}) {
	return func(payload map[string]interface{}) {
		...
		volunteer, indexingErr := index.IndexVolunteer(payload["userId"].(string), payload["accessToken"].(string))
		if indexingErr != nil {
			...
			log.Println("[INDEXING ERROR]", indexingErr)

			if volunteer != nil {
				addErrErr := volunteer.AddErr(indexingErr.Error())
				if addErrErr != nil {
					log.Println("[ERROR TRACK ERROR]", addErrErr)
				}
			}
		} else {
